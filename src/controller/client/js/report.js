function ListEqByPlaces(oid,pid,plpid){  
  ...
  jQuery("#list2").jqGrid({
   	url:'controller/server/report.php?curuserid='+plpid+'&curorgid='+oid+'&curplid='+pid+'&tpo='+$("#sel_rep :selected").val()+'&os='+$("#os").prop("checked")+'&mode='+$("#mode").prop("checked")+'&repair='+$("#repair").prop("checked"),
	datatype: "json",
...
function GetListPlaces(orgid,placesid){
       url="controller/server/getlistplaces.php?orgid="+orgid+"&placesid="+placesid+"&addnone=true";
       $("#sel_pom").load(url);
};

function GetListUsers(orgid,userid){
     $("#sel_plp").load("controller/server/getlistusers.php?orgid="+orgid+"&userid="+userid+"&addnone=true");
    };
...
newWin.document.write('<table id="list2">');
newWin.document.write($("#gbox_list2").html());
