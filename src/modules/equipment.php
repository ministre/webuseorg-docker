...
if (($user->mode==1) and ($step!=''))
{
    ...
    if ($step=="add")
    {
     if (count($err)==0)
     {               
        $sql="INSERT INTO equipment (id,orgid,placesid,usersid,nomeid,buhname,datepost,cost,currentcost,sernum,invnum,shtrihkod,os,mode,comment,active)
        VALUES (NULL,'$sorgid','$splaces','$suserid','$snomeid','$buhname','$dtpost','$cost','$currentcost','$sernum','$invnum','$shtrihkod','$os','$mode','$comment','1')";                                      
    ...
    if ($step=="edit")
    {
     if (count($err)==0)
     {
	      ...
	      orgid='$sorgid',placesid='$splaces',usersid='$suserid',nomeid='$snomeid',buhname='$buhname',
	      datepost='$dtpost',cost='$cost',currentcost='$currentcost',sernum='$sernum',invnum='$invnum',
	      shtrihkod='$shtrihkod',os='$os',mode='$mode',comment='$comment',photo='$picphoto' WHERE id='$id'";                                      
  		$result = mysql_query($sql,$cfg->base_id);                
    ...
    if ($step=="move")
    {
     if (count($err)==0)
     {
  		...
  		$result = mysql_query($sql,$cfg->base_id);                
  		if ($result==''){die('Не смог изменить регистр номенклатуры - перемещение!: ' . mysql_error());}
	...
	$sql="INSERT INTO move (id,eqid,dt,orgidfrom,orgidto,placesidfrom,placesidto,useridfrom,useridto,comment)
  	...
  	$result = mysql_query($sql,$cfg->base_id);
	//echo "!$sql!";
  	if ($result==''){die('Не смог добавить перемещение!: ' . mysql_error());}
		if ($cfg->sendemail==1){
		     $touser= new Tusers;
